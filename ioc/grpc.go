...
import (
	...
	grpc2 "github.com/dadaxiaoxiao/search/internal/grpc"
...
func InitGRPCServer(ecli *clientv3.Client,
	...
	redisClient redis.Cmdable, l accesslog.Logger) *grpcx.Server {
	type Config struct {
		Port    int   `yaml:"port"`
	...
	server := grpc.NewServer(
		grpc.ChainUnaryInterceptor(
			// prometheus 拦截器
			prometheus.NewInterceptorBuilder("qinye", "webook_search").BuildServer(),
			// 链路追踪
